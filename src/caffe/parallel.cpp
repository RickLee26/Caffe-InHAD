...
INSTANTIATE_CLASS(Params);
  ...
  PSWorker<Dtype>::PSWorker(shared_ptr<Solver<Dtype> > solver,
                ...
                manager_(manager)
                {
                  ...
                  } else if (Caffe::root_solver()) {
                    string filename = Caffe::header().record_file;
                    LOG(INFO) << "Rick Caffe Info name: " << filename;

                    collector.reset(new InfoCollector(filename));
                    solver_->collector.reset(collector.get());
                    solver_->net()->collector.reset(collector.get());
                    manager->collector.reset(collector.get());
                    collector->setDataSize(sizeof(Dtype), Caffe::solver_count() / (Caffe::header().mpi_count - 1), size_ + 1);
                  }
  ...
  void PSWorker<Dtype>::on_start(){
    if(local_version_ < (*global_version_)){
      if(collector.get())collector->Start();
      
      caffe_copy(size_, *g_data_, data_);
      local_version_ = *global_version_;

      if(collector.get())collector->addBatchTime(4);
    }
  }
  ...
  void PSWorker<Dtype>::on_gradients_ready(){
    ...
    if(collector.get())collector->addBatchTime(3);
  }
  ...
  void PSManager<Dtype>::run() {
    ...
    gb = new GradientBuffer<Dtype>(5, data_p1_len);
    int global_version = 0;

    int omp_count = local_solver_count + 2;
    // omp_count = 24 - local_solver_count;
    omp_count = 24;
    ...
    #pragma omp parallel num_threads(omp_count)
    {
      ...
      if(rank < local_solver_count){
        Caffe::header() = wh;
        // LOG(INFO) << wh.record_file << " : " << Caffe::header().record_file;
        ...
        while(new_comer || finish_count < local_solver_count){
          ...
          if (collector.get()) collector->increInfoComm();
          ...
          if(finish_count == local_solver_count || (status.MPI_TAG & MSG_HANDIN_DIFF)){
            // LOG(INFO) << "ohehe";
            diff = gb->swapCurrent(diff);
            ptr = send_recv(diff, MSG_HANDIN_DIFF, data_p1_len, status);
            if (collector.get()) collector->increDiffComm();
          }
  ...
  Dtype* PSManager<Dtype>::submit_diff(int rank, Dtype* diff){
    if (++iters[rank] == max_iter) ++finish_count;
    new_comer++;
